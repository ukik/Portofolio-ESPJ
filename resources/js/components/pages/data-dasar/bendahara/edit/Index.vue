<template>
    <div>
        <Toolbar judul="Edit Data Dasar Bendahara" slot="title">
        ...
        <Formulir>
            ...
            <FormulirLegend slot="legend" title="Edit Data Dasar Bendahara" />
            ...
            <FormulirInput 
                ...
                title="Nomor SK" 
                name="nomor_sk" 
                :value="get_form.nomor_sk" 
                placeholder="Nomor SK" />


            <FormulirPicker 
                @onPicker="onPicker({ key: 'tanggal_sk', value: $event })"
                slot="input" 
                type="date"
                title="Tanggal SK" 
                name="tanggal_sk" 
                v-model="get_form.tanggal_sk" 
...
export default {
    computed: {
        ...mapGetters({
            'get_listPegawai': 'getters/data-dasar/bendahara/listPegawai',
            'get_form': 'getters/data-dasar/bendahara/form',
        }),
    },
    methods: {
        ...mapActions({
            'set_listPegawai': 'setters/api/data-dasar/bendahara/listPegawai',            
        }),    
        ...
        onInitData(){
          ...
          const request = new Promise((resolve, reject) => {
              axios.get('data-dasar/bendahara/'+id+'/edit').then((response) => {
        ...
        onSubmit(){
          ...
          const payload = { id, pegawai_id, nomor_sk, tanggal_sk, aktif }

          const request = new Promise((resolve, reject) => {
              axios.put('data-dasar/bendahara/'+id, payload).then((response) => {
                  resolve(response)
              }).catch((error) => {
                  reject(error);
              })
