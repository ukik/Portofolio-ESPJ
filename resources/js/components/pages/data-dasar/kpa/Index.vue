<template>
    <div>
        <Toolbar :judul="judul" slot="toolbar">
            ...
            <AddButton route="data-dasar-kpa-create" name="KPA" class="mr-2" />
            <BackButton />
        </Toolbar>
        ...
        <TableView :items="get_kpa.data" :theads="theads" :action_visibility="user == 'Super Admin'">
            <template slot-scope="scope" v-if="get_kpa.data.length > 0">
              <tr>
                ...
                <td class="align-middle">{{ scope.item.singkatan }}</td>
                ...
                <td class="text-center" v-if="user == 'Super Admin'">
                    <a @click="set_form({ id: scope.item.id }); jump({ name:'data-dasar-kpa-edit' })" class="icon">
            ...
            <Paginator 
                slot="paginate" 
                :totalPage="total_paginate(get_kpa.last_page)" 
...
export default {
    computed: {
        ...mapGetters({
            'get_kpa': 'getters/data-dasar/kpa/kpa',
        }),
    },
    methods: {
        ...mapActions({
            'set_kpa': 'setters/api/data-dasar/kpa/kpa',
            'set_form': 'setters/non-api/data-dasar/kpa/form',  
        ...
        onInitData(){
            ...
            const request = new Promise((resolve, reject) => {
                axios.get('data-dasar/kpa'+this.suffix).then((response) => {
                    if(response == undefined) {
                        return resolve()
                    }
                                        
                    resolve(this.set_kpa(response.data.data.kpa))
                }).catch((error) => {
                    reject(error);
                })
            })
