<template>
    <div>
        <Toolbar :judul="judul" slot="toolbar">
            ...
            <AddButton route="data-dasar-lokasi-luar-negeri-create" name="Lokasi" class="mr-2" />
            <BackButton />
        </Toolbar>
            ...
            <template slot-scope="scope" v-if="get_lokasi.data.length > 0">
                <tr class="font-weight-bold">
                    ...
                    <td class="text-center" v-if="user == 'Super Admin'">
                        <a @click="set_create({ induk: scope.item.id, nama: scope.item.nama }); jump({ name:'data-dasar-lokasi-luar-negeri-create' })" class="icon"><i class="fe fe-plus"></i></a>
                        <a @click="set_form({ id: scope.item.id }); jump({ name:'data-dasar-lokasi-luar-negeri-edit' })" class="icon"><i class="fe fe-edit"></i></a>
                ...
                <tr v-for="(item, index) in scope.item.sub" :key="index+'-'+scope.index" >
                    ...
                    <td class="text-center" v-if="user == 'Super Admin'">
                        ...
                        <a @click="set_form({ child_id: item.id }); jump({ name:'data-dasar-lokasi-luar-negeri-edit' })" class="icon"><i class="fe fe-edit"></i></a>
...
export default {
    computed: {
        ...mapGetters({
            'get_lokasi': 'getters/data-dasar/lokasi/luar-negeri/lokasi',
        }),
    },
    methods: {
        ...mapActions({
            'set_lokasi': 'setters/api/data-dasar/lokasi/luar-negeri/lokasi',
            'set_form': 'setters/non-api/data-dasar/lokasi/luar-negeri/form',
            'set_create': 'setters/non-api/data-dasar/lokasi/luar-negeri/create',
            'set_splice': 'setters/non-api/data-dasar/lokasi/luar-negeri/splice',
        }),
        ...
        onInitData(){
            ...
            const request = new Promise((resolve, reject) => {
                axios.get('data-dasar/lokasi-luar-negeri'+this.suffix).then((response) => {
        ...
        onDelete(args){
            this.onDeleteConfirmation({
                url: 'data-dasar/lokasi-luar-negeri/'+args.id,
                id: args.id,
                bundle: args
            })       
        }          
    ...
    data() {
        return {
                ...
                judul: 'Data Dasar Lokasi Luar Negeri',
