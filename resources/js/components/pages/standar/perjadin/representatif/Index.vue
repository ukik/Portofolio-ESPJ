    ...
    <div>
        <Toolbar :judul="judul" slot="toolbar">
            ...
            <DropdownButton class="mr-2" classname="success" name="Standar Lainnya">
                ...
                <a v-pointer class="dropdown-item bg-success text-white" @click="jump({ name:'standar-perjadin-representatif' })">Representatif</a>
            ...
            <AddButton route="standar-perjadin-representatif-create" name="Lokasi" class="mr-2" />
            <BackButton />
        </Toolbar>
        ...
        <TableView :items="get_representatif.data" :theads="theads" :action_visibility="user == 'Super Admin'">
            <template slot-scope="scope">
                <tr>
                    ...
                    <td class="text-center" v-if="user == 'Super Admin'">
                        <a @click="set_form({ id: scope.item.id }); jump({ name:'standar-perjadin-representatif-edit' })" class="icon"><i class="fe fe-edit"></i></a>
            ...
            <Paginator 
                slot="paginate" 
                :totalPage="total_paginate(get_representatif.last_page)" 
...
export default {
    computed: {
        ...mapGetters({
            'get_representatif': 'getters/standar/perjadin/representatif',
        }),
    },
    methods: {
        ...mapActions({
            'set_representatif': 'setters/api/standar/perjadin/representatif',
            'set_form': 'setters/non-api/standar/perjadin/representatif/form',  
            'set_splice': 'setters/non-api/standar/perjadin/representatif/splice',
        }),
        ...
        onInitData(){
            ...
            const request = new Promise((resolve, reject) => {
                axios.get('standar/perjadin/representatif'+this.suffix).then((response) => {
                    if(response == undefined) {
                        return resolve()
                    }
                                        
                    resolve(this.set_representatif(response.data.data.representatif))
                }).catch((error) => {
                    reject(error);
                })
            })
        ...
        onDelete(args){
            this.onDeleteConfirmation({
                url: 'standar/perjadin/representatif/'+args.id,
                id: args.id,
                bundle: args
            })      
        }          
    ...
    data() {
        return {
            ...
            callback_status: this.$store.state.callback.callback_status,
                ...
                judul: 'Standar Perjalanan Dinas Representatif',
