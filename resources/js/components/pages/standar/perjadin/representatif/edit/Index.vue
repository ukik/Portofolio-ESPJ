<template>
    <div>
        <Toolbar judul="Edit Standar Perjadin Representatif" slot="title">
            <BackButton />
        </Toolbar>    
        ...
        <Formulir>
            ...
            <FormulirLegend slot="legend" title="Edit Standar Perjadin Representatif" />
...
export default {
    computed: {
        ...mapGetters({
            'get_form': 'getters/standar/perjadin/representatif/form',
        }),
    },
    methods: {   
        onSubmit(){
          ...
          const payload = { id, jabatan, nominal_luar_wilayah, nominal_dalam_wilayah }

          const request = new Promise((resolve, reject) => {
              axios.put('standar/perjadin/representatif/'+id, payload).then((response) => {
                  resolve(response)
              }).catch((error) => {
                  reject(error);
              })
